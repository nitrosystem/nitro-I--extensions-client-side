...
function SwiperSliderController(field, $scope, moduleController, $element) {
    var layoutTemplate;
    var itemTemplate;
    ...
    $scope.$watch('Field.' + field.FieldName + '.DataSource.Items', function (newVal, oldVal) {
        if (newVal != oldVal && newVal.length) {
            ...
            for (var i = 0; i < newVal.length; i++) {
                var template = itemTemplate.replace(/{Item\.(.[^}]+)}/gm,  field.Settings.DataSource.ListName + '[' + i + '].$1');
                var $sliderItem = $('<div class="swiper-slide"></div>');
                $sliderItem.append(moduleController.$compile(template)($scope));
